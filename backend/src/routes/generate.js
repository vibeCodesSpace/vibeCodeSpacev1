...
router.post('/generate-app', apiLimiter, authenticateUser, checkUsage, validateRequest, async (req, res) => {
    ...
    try {
        ...
        const { error: incrementError } = await supabase.rpc('increment_generations', { user_id: req.user.id });
        if (incrementError) {
            // If this fails, we should ideally handle it, but for now, we'll log it.
            console.error(`Failed to increment usage for user ${req.user.id}:`, incrementError);
        }
        ...
        res.status(500).json({ error: 'An error occurred during the application generation process.' });
    }
});
