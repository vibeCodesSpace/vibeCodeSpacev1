import { AIProvider, OpenAIProvider, AnthropicProvider, GeminiProvider, GeneratedSite, ResumeData } from './providers';
...
export class AIManager {
  ...
  async generateSite(prompt: string, preferredProvider?: string): Promise<GeneratedSite> {
  ...
  async generateComponent(description: string, preferredProvider?: string): Promise<string> {
    const provider = this.getProvider(preferredProvider);
  ...
  async extractResumeData(resumeText: string, preferredProvider?: string): Promise<ResumeData> {
    const provider = this.getProvider(preferredProvider);
  ...
  async generatePortfolioFromResume(resumeData: ResumeData): Promise<GeneratedSite> {
...
Make it modern, professional, and visually appealing with sections for About, Experience, Skills, Education, and Contact.`;
  ...
  private getProvider(preferredProvider?: string): AIProvider {
    ...
    if (!provider) {
      throw new Error(`Provider ${providerName} not available. Available providers: ${Array.from(this.providers.keys()).join(', ')}`);
    }
